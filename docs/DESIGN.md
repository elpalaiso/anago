...
- **`--install-timer` / `--uninstall-timer`**는 상호 배타다. 리눅스면
  systemd 유닛(`anago-sync.service` + `anago-sync.timer`), 맥이면
  launchd를 설치한다. **양쪽 다 시스템 수준(root)**이다 — sync가 wg
  설정을 적용하려면 root여야 하므로 맥에서도 사용자 LaunchAgent가 아니라
  `/Library/LaunchDaemons`의 데몬이다.
...
- 타이머가 설치됐는지 보는 별도 표면은 두지 않는다. 설치 명령이 확인
  방법(`systemctl list-timers` / `launchctl list`)을 출력하면 충분하고,
  §6.3대로 timer는 편의이지 정합성 요건이 아니다.
